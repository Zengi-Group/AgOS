...
-- Спек: Docs/AGOS-TSP-PoolClose-ConfirmBothForms-ARS-731.md (G2 2026-09-18)
--
-- Дефект: сделка фермер↔комбинат записывается двумя формами — ссылкой в партии
-- (batches.pool_line_id, авто-матч) и строкой в batch_allocations (ручной матч).
-- Все ЧЕТЫРЕ тела, закрывающие заявку по набору цели, подтверждали только СВОЮ
-- форму и оставляли чужую в `matched` — тупик обеих сторон (у фермера нет хода
-- отгрузки, у комбината строка врёт «Ожидает отгрузки»).
--
-- FR-001: односторонний `update` в каждом из четырёх тел ЗАМЕНЁН вызовом уже
--         задеплоенной fn_tsp_pool_confirm_matches (ARS-695) — один дом правила (P4).
-- FR-002: вызов внутри ветки ФАКТИЧЕСКОГО перехода (`if found` после
--         `update pools … where status = 'filling'`); повтор безопасен
--         (идемпотентность по `where status = 'matched'`).
-- FR-003: сигнатуры не меняются, новых функций нет (P7) — правятся только тела.
-- FR-013: новая миграция ПОВЕРХ 20260726140000 / 20260702190000; ни одна
--         применённая миграция не переписывается на месте. Этих четырёх функций
--         нет в d02_tsp.sql вовсе, поэтому правило «всё в канонические файлы»
--         здесь неприменимо (guard TSP-SLICE9-ROLLBACK-01 запрещает гнать d02 целиком).
-- FR-011: rpc_self_match_batch_to_pool НЕ трогается — правится хелпер
--         fn_tsp_alloc_chunk, который она зовёт, но не она сама.
-- FR-016: вызов добавляет эмиссию batch.confirmed на четырёх путях — принято
--         сознательно; попутно чинится ложное `partial_accept: true` в метаданных.
-- ═══════════════════════════════════════════════════════════════════════════════

-- ── 0. fn_tsp_pool_confirm_matches — честное partial_accept (FR-016) ─────────
-- Тело ARS-695 (20260914120000:116) без изменений, кроме метаданных события:
-- жёсткое `partial_accept: true` было верно, пока функцию звал ТОЛЬКО путь
-- частичного принятия. С FR-001 её зовут ещё четыре пути полного набора, где
-- это прямая ложь в журнале. Значение вычисляется из факта: цель набрана или нет.
-- Считается ДО смены статуса заявки, потому что оба вызывающих ARS-695 зовут
-- функцию ПЕРЕД `update pools set status` — судить по статусу было бы неверно.
...
declare
    ...
    v_partial boolean;
begin
    -- ARS-731 (FR-016): цель набрана или нет — считаем ФАКТОМ из счётчиков заявки,
    -- а не из её статуса: оба вызывающих ARS-695 зовут эту функцию ДО `update pools
    -- set status`, а четыре пути ARS-731 — ПОСЛЕ. Счётчик matched_heads к моменту
    -- вызова инкрементирован на всех шести путях, поэтому он и есть общий знаменатель.
    select coalesce(p.matched_heads, 0) < coalesce(p.target_heads, 0)
      into v_partial
    ...
    loop
        ...
        values (v_bid, 'confirmed',
            jsonb_build_object('pool_id', p_pool_id, 'partial_accept', v_partial,
                               'route', 'batch'),
...
comment on function public.fn_tsp_pool_confirm_matches(uuid) is
    'ARS-695 (FR-003/FR-005) | Подтверждение матчей заявки по ОБОИМ маршрутам: куски
     ...
     ARS-731 (FR-016): partial_accept в метаданных события вычисляется из счётчиков
     заявки, а не зашит в true — функцию зовут и пути ПОЛНОГО набора.';
revoke execute on function public.fn_tsp_pool_confirm_matches(uuid) from public, anon, authenticated;


-- ── 1. rpc_self_activate_pool_request — свип активации (FR-001/FR-002) ───────
    ...
    loop
        ...
        if found then
            ...
            if (v_line.matched_heads + v_batch.heads) >= v_line.target_heads then
                ...
                -- ARS-731 (FR-001/FR-002): односторонний `update batches` ЗАМЕНЁН вызовом —
                -- он подтверждал только форму «ссылка в партии» и оставлял в `matched`
                -- строки batch_allocations этой же заявки. `if found` добавлен здесь
                -- впервые: это единственный из четырёх путей, где подтверждение шло
                -- БЕЗ проверки фактического перехода заявки в closed_filled.
                if found then
                    perform public.fn_tsp_pool_confirm_matches(v_pool_id);
                end if;
...
-- ── 2. rpc_self_auto_match_batch — авто-матч при публикации (FR-001/FR-002) ──
...
begin
    ...
    if found then
        ...
        if (v_line.matched_heads + v_batch.heads) >= v_line.target_heads then
            ...
            if found then
                -- ARS-731 (FR-001): вместо одностороннего `update batches` — вызов,
                -- подтверждающий ОБЕ формы записи (ссылка в партии + строки аллокаций).
                perform public.fn_tsp_pool_confirm_matches(v_line.pool_id);
            end if;
        end if;
...
-- ── 3. rpc_self_accept_offer — принятие оффера фермером (FR-001/FR-002) ──────
...
begin
    ...
    if (v_line.matched_heads + v_batch.heads) >= v_line.target_heads then
        ...
        if found then
            -- ARS-731 (FR-001): вместо одностороннего `update batches` — вызов,
            -- подтверждающий ОБЕ формы записи. Именно этот путь при счёте «два тела»
            -- остался бы сломанным: заявка, закрытая принятием оффера, оставляла
            -- чужую форму в `matched`.
            perform public.fn_tsp_pool_confirm_matches(v_line.pool_id);
        end if;
    end if;
...
-- ── 4. fn_tsp_alloc_chunk — аллокатор куска (FR-001/FR-002/FR-011) ───────────
...
declare
    ...
    v_vol       int;
    v_full      boolean;
    -- ARS-731: v_bid убран — курсор rollup'а уехал внутрь fn_tsp_pool_confirm_matches,
    -- переменная осталась бы мёртвой (HS-4).
begin
    select * into v_batch from public.batches   where id = p_batch_id     for update;
    ...
    if (v_pool.matched_heads + v_take) >= v_pool.target_heads then
        ...
        if found then
            -- ARS-731 (FR-001): зеркальная половина того же дефекта — здесь подтверждались
            -- только куски (batch_allocations) и оставались в `matched` партии, привязанные
            -- ссылкой (batches.pool_line_id) авто-матчем. Вызов делает ОБЕ формы; rollup
            -- статуса батча живёт внутри него же и по-прежнему оставляет партию с остатком
            -- на рынке в partially_matched.
...
comment on function public.fn_tsp_alloc_chunk(uuid, uuid, text, uuid, int, int) is
    'Слайс 9 (+S3) | Аллокатор куска батча в строку пула. take=min(остаток,свободно строки/
     ...
     ARS-731 (FR-001) — подтверждение ОБЕИХ форм записи через fn_tsp_pool_confirm_matches
     (куски + партии, привязанные ссылкой), rollup статуса батча внутри неё
