...
-- =============================================================================
-- ARS-646 / MP-2.2 — rpc_get_mpk_profile_overview: единственное чтение вкладки «Обзор».
-- Спека: Docs/AGOS-MPK-Profile-OverviewRPC-ARS-646.md — действующие FR-001…FR-018 · FR-023 /
-- M-001…M-014 · M-020. FR-019…FR-022 и M-015…M-019 в отставке 2026-09-09 вместе со счётом
-- сделок (решение владельца «не застревать»): три сделочных числа отдаются признаком
-- «пока не считается», а не цифрой (FR-023 / M-020).
-- Контракт ответа — Docs/AGOS-Dok3-RPC-Catalog-v1_5.md RPC-64 (D-RPC-CONTRACT-SYNC-01).
--
-- Почему SECURITY DEFINER: org_field_reviews отозвана у роли authenticated слайсом ARS-359,
-- а deal_reviews закрыта RLS, которая по конструкции не умеет отдать СКРЫТЫЙ отзыв (её
-- предикат fn_ars_360_can_read_revealed_deal_review пускает только полностью раскрытые
-- пары). Пункт «скрытый отзыв» (M-008) без definer недостижим в принципе, а не «неудобен».
-- Мёртвые SELECT-политики остаются домом ARS-358 (FR-011 спеки ARS-362), не этого слайса.
--
-- ДВА ПУТИ ДОСТУПА И НИКАКИХ ДРУГИХ (FR-002 → ARS-362 FR-015, ссылка, не второй дом):
-- членство в запрошенной организации (fn_my_org_ids) либо админ TURAN (fn_is_admin).
-- СЛУЖЕБНОГО ПУТИ НЕТ — слагаемого coalesce(auth.role(),'') = 'service_role' нет ни в гейте
-- сессии, ни в предикате владения. Грант execute у service_role СОХРАНЁН по той же причине,
-- что у соседа: без него служебный вызов падал бы сырой ошибкой прав Postgres мимо
-- обработчика, а с ним доходит до гейта и получает типизированный отказ (M-004).
--
-- ЧУЖИЕ ФАКТЫ БЕРУТСЯ У ИХ ХОЗЯЕВ, А НЕ ПЕРЕВЫВОДЯТСЯ (P4). Членство и верификация —
-- rpc_get_org_membership_verification (ARS-361, задеплоен); репутация —
-- rpc_get_mpk_reputation (ARS-360, задеплоен). Оба stable, оба уже несут свои гейты;
-- предикат владения здесь стоит ДО них, поэтому их отказ недостижим. Своё выводится только
-- то, у чего дома нет: правки критических полей, скрытые отзывы, число сотрудников.
--
-- АТРИБУЦИЯ «НАША СДЕЛКА» (M-008) — правило двух маршрутов задеплоенного
-- fn_ars_360_mpk_counterparty_ids, но путём владения из FR-016 (pool_requests, НЕ
-- pools.organization_id: колонки нет в этом каноническом файле, опираться на необъявленное
-- нельзя; расхождение двух путей — долг ARS-668, на живых данных 0 расхождений на 23 пулах).
-- Тело чужой функции НЕ переиспользуется: её разрез «партия → массив МПК», здесь нужен
-- обратный, и она revoke all от всех ролей. Дублируется ПРАВИЛО, не код; расхождение двух
-- реализаций — дефект, сторож — M-008 в тестах.
-- Сделочные таблицы читаются ТОЛЬКО ради этой атрибуции. В блоке facts сделочных join'ов
-- нет и быть не должно (FR-023): три числа приходят признаком.
--
-- ЛОВУШКИ СНАПШОТА CHECK 11, обе уже стоили ложно-зелёной проверки у соседа:
--   1) IMPL_DEBT CONTRACT-SNAPSHOT-DOTTED-KEYS-01 — комментарий ВНУТРИ списка аргументов
--      jsonb_build_object склеивается с ключом, и ключ молча исчезает из снапшота.
--      Комментарии держать ВЫШЕ вызова. Ключ "mpk.review.submit" снапшот не видит вовсе
--      (точка в имени) — его сторожат только тесты.
--   2) IMPL_DEBT CONTRACT-SNAPSHOT-TOJSONB-ROWS-01 — to_jsonb(row) не даёт снапшоту ни
--      одного ключа. Все объекты собираются ЯВНЫМИ jsonb_build_object.
--
-- Аддитивно (FR-009, P7): новых таблиц и колонок нет, ни одна существующая сигнатура не
-- тронута, писатели не тронуты. Индексов не добавлено, и утверждение точное: выборку правок
-- покрывает задеплоенный ARS-359 idx_org_field_reviews_org_status; атрибуцию сделок —
-- idx_batch_alloc_pool / idx_batch_alloc_status и первичные ключи пулов; отзывы —
-- idx_deal_reviews_batch_id. НЕ покрыт индексом обход pool_requests по organization_id —
-- набор мал по конструкции (11 организаций с заявками на проде), спекулятивного индекса нет
-- (HS-4). Цифры замера и его условия — в пакете G3, не в комментарии: комментарий устаревает
-- при первой правке фикстуры и начинает врать про замер, которого не было.
-- Читатель ничего не мутирует и не эмитит событий (FR-015) — держится на STABLE.
-- =============================================================================
