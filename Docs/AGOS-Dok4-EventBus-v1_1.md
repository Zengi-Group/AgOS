...
| **market.batch.confirmed** *(EMITTED — 2026-09-14, ARS-695: путь частичного принятия; долг TSP-FLOW-05 закрыт наполовину)* | `fn_tsp_emit_batch_confirmed` из `rpc_self_pool_accept_partial` (обоим маршрутам матча) · **путь полного набора всё ещё НЕ эмитит** (`fn_tsp_alloc_chunk` / `rpc_self_accept_offer` — заморожены FR-001, остаток долга держит ARS-314) | Farmer+MPK Notification ✅ (D-M6-5: identity revealed), Audit | batch_id, pool_id, mpk_org_id, mpk_legal_name, farmer_org_id, farmer_legal_name |
...
| **market.pool.closed_partial** *(EMITTED — 2026-09-14, ARS-695)* | `rpc_self_pool_accept_partial` (рабочий путь) · канонический `rpc_pool_accept_partial` выведен из пути и revoked, FR-013 | Farmer Notification ✅ (matched batches confirmed), Audit | pool_id, confirmed_units, matched_heads, original_target_heads |
| **market.pool.closed_unfilled** *(EMITTED — 2026-09-14, ARS-695)* | `rpc_self_pool_return_batches` (решение МПК) · `rpc_self_close_due_pools` — ленивое подметание: набрано ниже `min_pool_heads` (`reason: below_min_heads`) либо истекло окно решения (`reason: decision_window_elapsed`, дефолт «вернуть» D-M6-1). Планировщика нет — ARS-694 заблокирован ARS-264, поэтому возврат происходит при первом заходе в кабинет, а не в минуту истечения окна | Farmer Notification ✅ (batches → published), Audit | pool_id, reason, returned_units \| matched_heads + min_pool_heads \| window_hours |
