...
| F10 ✅ | ARS-286 | semantic | offline-слой по §8 (кэш-юниты + outbox + реплей); построен, см. D148 | все экраны открываются из кэша с меткой; очередь переживает перезапуск — верифицировано вживую (offline-реплей 3 отклонений на seed-фермере, drainOutbox on-reconnect) |
...
| **D148** | F10 (ARS-286) построен: носитель — существующий `platform/storage.ts` `appStorage` (KVStorage, web=localStorage/Capacitor-swap), НЕ IndexedDB; live-канал НЕ подключаем в v1 (базовый механизм — рефетч по фокусу/PTR, как у существующих экранов); ещё-не-синканные `log_animal_event` читаются прямо из очереди outbox (не из onDone-состояния компонента) — строка гаснет сама через notify() при успешном дренаже, без ручного дедупа; «Осмотр»/«Ветврачу» отключены на таких строках (event_id = client opId, не существует на сервере, действие упало бы) | IndexedDB — второй storage-паттерн без нативной пары (P4); live-канал — своя мини-фича, ничего в F5–F9 её не использует (HS-4); onDone-состояние для оптимистичной строки — теряется при ремаунте и рискует задвоиться с реальной строкой после реплея (живой репро при разработке) | Оба решения D145 оставлял открытыми («выбор носителя — F10, human-led»); `getPendingItems`/`getFailedItems` (outbox.ts) — общий паттерн для любого будущего экрана с очередью, без новой абстракции сверху |
