...
- **Chatscope (R-19, решение CEO 2026-07-11 «дизайн чатов очень плохой → готовая библиотека»):** ConsultantScreen переведён на `@chatscope/chat-ui-kit-react` (ChatContainer/MessageList/Message/MessageInput/TypingIndicator) + тема `messages-chatscope.css` (перекраска `cs-*` в daylight; гоча: фон инпута сидит на `__content-editor-container`, не только wrapper/editor). Кнопки kit'а (FontAwesome) отключены — свои PhIcon mic/send; мок голосовой волны сохранён. `IonShellFrame` +`noScroll` (scrollY=false — у MessageList свой скролл, иначе двойной). Гоча: зависимость ставить при остановленном dev-сервере или чистить `node_modules/.vite` — иначе дубль React («Invalid hook call» из kit-компонентов). Peer-чек D-DEP-BUMP-01 пройден (react 18.3.1 deduped; единственный invalid — известный react-router v5-остров). Транспорт реального Консультанта — решение: `@ai-sdk/react` useChat к нашему AI Gateway (отдельная задача).
