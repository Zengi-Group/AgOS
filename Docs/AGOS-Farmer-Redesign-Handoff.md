...
### 🚧 Phase 4 — Рынок (В РАБОТЕ: 4a+4b+4c+4d готовы, проверены; 4e осталось)
...
- **Phase 4 — Рынок:** осталось 4e (Review+Turan тела) — см. блок «🚧 Phase 4» выше (4d Wizard закрыт в Сессии 3). 4e: `ReviewScreen`/`TuranScreen` тела → `.mk-stars`/`.mk-rev-q`/`.mk-res`/`.mk-field`/`.mk-seltrig` + PhIcon вместо emoji (📞⏰✓); переиспользовать новые атомы `MkSelect`/`MkField`/`CheckRow`. Сохранить `rpc_submit_review`.
