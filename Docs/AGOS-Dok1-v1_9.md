...
| D-ARS229 | Доска спроса МПК (фермер) = агрегат-only; раскрытие медиа/ИНЖ = после закрытия пула | **Часть 1 (спрос):** `rpc_get_demand_board` над M6-поверхностью (`pools` status=`filling` + `pool_lines` is_active + `pool_regions`), НЕ pre-M6 `pool_requests`. Показывает категорию · регион (район свёрнут до области, чтобы не деанонимизировать одинокого МПК) · индикативную цену (min/max/avg) · объём · число заявок. НИКОГДА личность МПК (Art.171 aggregate-only; обязательный дисклеймер под списком). **Часть 2 (раскрытие ARS-227/228):** матч-МПК получает read-only доступ к `batch_media` + `batch_animals` через `fn_batch_revealed_to_me` (RLS в d02 + storage RLS в d10). Reveal = **D-M6-5/12** (`batches.status ∈ confirmed\|dispatched\|delivered`, т.е. пул `closed_filled`/`closed_partial`), НЕ D40 (`executing`). Write остаётся у владельца+админа. ARS-10 (юр. sign-off) — гейт перед публичным запуском доски, не перед кодом; строим за дисклеймером. |
