...
**Итог (исправлен 2026-09-03 при заходе на MP-3.3):** таблица выше перечисляет для `org`
**только писателей** — прежняя формулировка «4 из 6 подразделов имеют полное задеплоенное
покрытие» была неверна и завела MP-3.3 в нестроимое состояние. Фактически:

- **чтение `org` отсутствует полностью.** Читателя нет ни одного, а таблицы
  `mpk_profiles`/`mpk_sites`/`org_bank_accounts`/`org_field_reviews` закрыты от роли
  `authenticated` (`revoke all` в `d01_kernel.sql`; SELECT-политики для неё существуют, но
  без гранта не срабатывают). UI читает по `org` только то, что даёт `rpc_get_my_context`:
  `legal_name`, `bin_iin`, `region_id`, `phone`. Юр. адрес, e-mail, площадка, банк,
  pending-правки и карточка для фермера недостижимы. Дом читателя — **ARS-362**
  (`rpc_get_org_profile`).
- **карточки организации для фермера не существует ни в каком виде** — ни закрытой, ни
  раскрытой: `buyer`/`buyerPhone` в `src/pages/cabinet/shell/types.ts` — тип без бэкенда,
  токен `buyer` не встречается ни в одном SQL-файле. Поэтому «одна read-model на превью и
  на фермерскую выдачу» (`FR-007`, EngSpec §8) пока не с чем разделять.

Дыры, как они есть: **чтение `org`** · **агрегат Обзора** (обе — ARS-362) · **Обращения**
(нет ни модели, ни RPC). Плюс открытый гейт `ARS-358` (RLS/grants) — спека требует закрыть
его до отгрузки поверхности репутации. Полностью покрыты записью+чтением: `adm` (частично),
`team`, `rep`.
