# AGOS — Dok 3: RPC Catalog — Version 1.5

**Project:** TURAN Agricultural Operating System
**Version:** 1.5
**Date:** 22 June 2026
**Status:** АКТУАЛЬНЫЙ — прошёл Architecture Audit + Schema Consolidation + Doc Reconciliation Phase 1
**Depends on:** Dok 1 (v1.9), Dok 4 (Event Bus v1.1), Dok 5 (v1.7)
...
**Changelog v1.4 → v1.5 (Doc Reconciliation Phase 1, 2026-06-22):**

| # | Тип | Изменение |
|---|-----|-----------|
| FEED-03 | 🔴 | RPC-21,22,23,24: 📋 Planned → ✅ Implemented |
| VET-05 | 🔴 | RPC-26,27,28,29,31,32: 📋 Planned → ✅ Implemented |
| OPERATIONS-04 | 🔴 | RPC-37 (`rpc_get_active_plan`): 📋 Planned → ✅ Implemented |
| OPERATIONS-05 | 🔴 | RPC-44 (`rpc_add_knowledge_chunk`): 📋 Planned → ✅ Implemented |
| FEED-01 | 🔴 | RPC-21 return shape corrected: `{inventory_id, is_new, quantity_kg, confidence}` |
| FEED-02 | 🔴 | RPC-24 return shape corrected: `jsonb[]` farm-level array (D-S3-2) |
| CONSULTING-02 | 🔴 | `rpc_save_consulting_ration` RETURNS corrected: `jsonb {ration_version_id, version_number}` |
| OPERATIONS-06 | 🔴 | RPC-33 params corrected: `p_template_id`, `p_plan_start_date`; note S-1 |
| OPERATIONS-09 | 🔴 | RPC-36 return col corrected: `old_start` (was `current_start`); added `old_end/new_end` |
| VET-05 | 🔵 | Added: `rpc_get_vet_case_detail` (D-F11-1) + 4 READ-RPCs (vaccination plans/items/vaccines/signals) |
| AI-GATEWAY-07 | 🔵 | Added: 9 deployed AI tool surface entries (AI-23..AI-31) in §1.8 + §10 |
| IDENTITY-05 | 🔵 | RPC-01: added note on canon split (Microstep1 D-IDM-6) vs current atomic form |
...
| ID | Функция | Домен | Вызов | Статус | Возвращает |
|----|---------|-------|-------|--------|------------|
| RPC-21 | `rpc_upsert_feed_inventory` | Feed | web, ai | ✅ Implemented | jsonb { inventory_id, is_new, quantity_kg, confidence } |
| RPC-22 | `rpc_save_ration` | Feed | web, ai | ✅ Implemented | jsonb { ration_id } |
| RPC-23 | `rpc_archive_ration` | Feed | web, ai | ✅ Implemented | boolean |
| RPC-24 | `rpc_get_current_ration` | Feed | web, ai | ✅ Implemented | jsonb[] (farm-level; one element per active ration, D-S3-2) |
...
| RPC-26 | `rpc_add_vet_diagnosis` | Veterinary | web | ✅ Implemented | uuid (diagnosis_id) |
| RPC-27 | `rpc_add_vet_recommendation` | Veterinary | web | ✅ Implemented | uuid (recommendation_id) |
| RPC-28 | `rpc_close_vet_case` | Veterinary | web | ✅ Implemented | boolean |
| RPC-29 | `rpc_create_vaccination_plan` | Veterinary | web, ai | ✅ Implemented | uuid (plan_id) |
| RPC-30 | `rpc_add_vaccination_plan_item` | Veterinary | web | 📋 Planned | uuid (plan_item_id) |
| RPC-31 | `rpc_record_vaccination` | Veterinary | web, ai | ✅ Implemented | uuid (record_id) |
| RPC-32 | `rpc_report_epidemic_signal` | Veterinary | ai, admin | ✅ Implemented | uuid (signal_id) |
| RPC-VET-R1 | `rpc_get_vet_case_detail` | Veterinary | web | ✅ Implemented | jsonb (case + symptoms + diagnoses + recommendations) |
| RPC-VET-R2 | `rpc_list_vaccination_plans` | Veterinary | web | ✅ Implemented | jsonb[] [{ plan_id, protocol_name, status, target_count, completed_count }] |
| RPC-VET-R3 | `rpc_list_vaccination_plan_items` | Veterinary | web | ✅ Implemented | jsonb[] [{ item_id, vaccine_id, vaccine_name, herd_group_id, due_date, status }] |
| RPC-VET-R4 | `rpc_list_vaccines` | Veterinary | web, ai | ✅ Implemented | jsonb[] [{ vaccine_id, name_ru, disease_target, withdrawal_days }] |
| RPC-VET-R5 | `rpc_list_epidemic_signals` | Veterinary | web, admin | ✅ Implemented | jsonb[] [{ signal_id, region_id, signal_type, severity, reported_at, status }] |
...
| RPC-37 | `rpc_get_active_plan` | Operations | web, ai | ✅ Implemented | jsonb |
...
| RPC-44 | `rpc_add_knowledge_chunk` | Platform | admin | ✅ Implemented | uuid (chunk_id) |
...
| AI-23 | `rpc_cancel_batch` | `cancel_batch` | boolean |
| AI-24 | `rpc_get_price_for_sku` | `get_price_for_sku` | jsonb { reference_price, minimum_price, disclaimer_text } |
| AI-25 | `rpc_get_market_summary` | `get_market_summary` | jsonb { supply_by_sku[], demand_by_sku[], price_grid_summary[] } |
| AI-26 | `rpc_close_vet_case` | `close_vet_case` | boolean |
| AI-27 | `rpc_get_farm_summary` | `get_farm_summary` | jsonb { farm_name, herd_count, active_plans_count, open_vet_cases_count } |
| AI-28 | `rpc_get_current_ration` | `get_current_ration` | jsonb[] (farm-level; = RPC-24 via AI context) |
| AI-29 | `rpc_upsert_feed_inventory` | `update_feed_inventory` | jsonb { inventory_id, is_new, quantity_kg, confidence } (= RPC-21 via AI context) |
| AI-30 | `rpc_log_herd_event` | `log_herd_event` | uuid (event_id) |
| AI-31 | `rpc_get_active_plan` | `get_active_plan` | jsonb { plan, phases[], current_phase } (= RPC-37 via AI context) |

> ⚠️ **Nota bene для vibecoding:** AI-02 = RPC-06, AI-05 = RPC-34, AI-07 = RPC-25, AI-20 = RPC-09, AI-21 = RPC-10, AI-28 = RPC-24, AI-29 = RPC-21, AI-31 = RPC-37. Одна и та же SQL функция — разные контексты вызова. Для AI Gateway используйте AI-XX, для Web-кабинета — RPC-XX.
>
> **AI-23..AI-31 (AI-GATEWAY-07, 2026-06-22):** Deployed tool surface паре с Dok 5 §6 (A3). Эти AI-инструменты задокументированы здесь; детальное описание LangGraph nodes — в Dok 5 §6.
...
> **IDENTITY-05 (debt):** Канонический дизайн (Microstep 1, D-IDM-6) предполагает три отдельных RPC: `rpc_register_user` (создание Supabase Auth user), `rpc_create_organization` (создание Organization), `rpc_submit_membership_application` (заявка на членство). Атомарная форма `rpc_register_organization` — текущая реальность. Разделение — impl debt Phase-2.
...
### RPC-21 `rpc_upsert_feed_inventory` [WEB] [AI] ✅ Implemented

Обновление запасов кормов на ферме (Layered Truth: data_source определяет confidence) → `jsonb { inventory_id, is_new, quantity_kg, confidence }`
...
### RPC-22 `rpc_save_ration` [WEB] [AI] ✅ Implemented

Сохранение варианта рациона → jsonb { ration_id }

*Создаёт новую RationVersion. Предыдущие версии сохраняются (append-only per D51).*

### RPC-23 `rpc_archive_ration` [WEB] [AI] ✅ Implemented

Архивирование рациона → boolean

### RPC-24 `rpc_get_current_ration` [WEB] [AI] ✅ Implemented

Текущий активный рацион на уровне фермы → `jsonb[]` (один элемент на активный рацион; D-S3-2: ферма может иметь несколько активных рационов по категориям животных). Каждый элемент содержит `ration`, `last_version`, `nutrient_summary`.
...
### RPC-26 `rpc_add_vet_diagnosis` [WEB] ✅ Implemented

Добавление диагноза ветеринаром → uuid (diagnosis_id)

### RPC-27 `rpc_add_vet_recommendation` [WEB] ✅ Implemented
...
### RPC-31 `rpc_record_vaccination` [WEB] [AI] ✅ Implemented
...
### RPC-32 `rpc_report_epidemic_signal` [AI] [ADMIN] ✅ Implemented

Регистрация эпидемического сигнала → uuid (signal_id)

### RPC-VET-R1 `rpc_get_vet_case_detail` [WEB] ✅ Implemented

Детали ветеринарного случая → jsonb (полный снимок: case + symptoms + diagnoses + recommendations + treatments)

> **D-F11-1 (VET-05):** Экран F11 (vet case detail page) требует самостоятельного READ-RPC с изоляцией по `organization_id`. Web JWT caller; RLS применяется автоматически.

| Параметр | Тип | Обяз. | Описание |
|----------|-----|-------|----------|
| `p_organization_id` | uuid | ✓ | Владелец — для явной изоляции поверх RLS |
| `p_vet_case_id` | uuid | ✓ | Ветеринарный случай |

### RPC-VET-R2 `rpc_list_vaccination_plans` [WEB] ✅ Implemented

Список планов вакцинации фермы → jsonb[] `[{ plan_id, protocol_name, status, target_count, completed_count, created_at }]`

| Параметр | Тип | Обяз. | Описание |
|----------|-----|-------|----------|
| `p_organization_id` | uuid | ✓ | — |
| `p_farm_id` | uuid | ✓ | — |
| `p_status` | text | — | filter: active \| completed \| archived |

### RPC-VET-R3 `rpc_list_vaccination_plan_items` [WEB] ✅ Implemented

Пункты плана вакцинации → jsonb[] `[{ item_id, vaccine_id, vaccine_name, herd_group_id, due_date, status, record_id? }]`
...
### RPC-VET-R4 `rpc_list_vaccines` [WEB] [AI] ✅ Implemented

Справочник вакцин (из `vet_products` с type='vaccine') → jsonb[] `[{ vaccine_id, name_ru, disease_target, withdrawal_days }]`

| Параметр | Тип | Обяз. | Описание |
|----------|-----|-------|----------|
| `p_is_active` | boolean | — | default true |

### RPC-VET-R5 `rpc_list_epidemic_signals` [WEB] [ADMIN] ✅ Implemented

Эпидемические сигналы → jsonb[] `[{ signal_id, region_id, signal_type, severity, reported_at, status }]`

| Параметр | Тип | Обяз. | Описание |
|----------|-----|-------|----------|
| `p_organization_id` | uuid | ✓ | Для admin: может быть NULL (показывать все); для web: обязателен |
| `p_status` | text | — | active \| resolved |
...
> **S-1 (OPERATIONS-06):** нет явного `p_organization_id` — `organization_id` разрешается через `actor_id → farm_members → organization_id`. Исключение из конвенции P-AI-2, задокументировано.

| Параметр | Тип | Обяз. | Описание |
|----------|-----|-------|----------|
| `p_farm_id` | uuid | ✓ | Ферма |
| `p_template_id` | uuid | ✓ | Шаблон производственного цикла (канонич. имя, OPERATIONS-06) |
| `p_plan_start_date` | date | ✓ | Дата начала плана (канонич. имя, OPERATIONS-06) |
...
Превью каскада без изменений → TABLE (phase_id, phase_name, old_start, new_start, old_end, new_end, shift_days, date_type, depth)

> **OPERATIONS-09:** Колонка `old_start` (не `current_start` — исправлено в v1.5). Добавлены `old_end` и `new_end` для отображения полного сдвига окна фазы в UI.

*Показать фермеру список что изменится → он подтверждает → вызов RPC-35.*

### RPC-37 `rpc_get_active_plan` [WEB] [AI] ✅ Implemented
...
### RPC-44 `rpc_add_knowledge_chunk` [ADMIN] ✅ Implemented
...
| `rpc_get_production_plan` | rpc_get_active_plan | ✅ Resolved v1.5 | RPC-37 = web version (✅ Implemented); AI-06 = AI version (both implemented; same function); AI-31 = new AI alias |
...
**Sprint 2: Veterinary (core AI use case)** ✅ DONE (v1.5)
- ~~RPC-26 `rpc_add_vet_diagnosis`~~ ✅ Implemented
- ~~RPC-27 `rpc_add_vet_recommendation`~~ ✅ Implemented
- ~~RPC-28 `rpc_close_vet_case`~~ ✅ Implemented
- ~~RPC-29 `rpc_create_vaccination_plan`~~ ✅ Implemented
- ~~RPC-31 `rpc_record_vaccination`~~ ✅ Implemented
- ~~RPC-32 `rpc_report_epidemic_signal`~~ ✅ Implemented

**Sprint 3: Feed & Nutrition** ✅ DONE (v1.5)
- ~~RPC-21 `rpc_upsert_feed_inventory`~~ ✅ Implemented
- ~~RPC-22 `rpc_save_ration`~~ ✅ Implemented
- ~~RPC-23 `rpc_archive_ration`~~ ✅ Implemented
- ~~RPC-24 `rpc_get_current_ration`~~ ✅ Implemented

**Sprint 4: Operations** ✅ DONE (v1.5)
- ~~RPC-37 `rpc_get_active_plan`~~ ✅ Implemented
...
rpc_save_consulting_ration(
    ...
    p_items                     jsonb,
    p_results                   jsonb
) RETURNS jsonb  -- { ration_version_id uuid, version_number int }  (CONSULTING-02)
...
|---------|-----|----------|
| `calculate_ration` | FastAPI + Supabase Edge | ✅ Deployed. NASEM LP-оптимизация рациона. Input: animal_category, weight, feeds[]. Output: RationVersion с нутриентным балансом |
| `get_feed_budget` | FastAPI | ✅ Deployed. Годовая потребность и дефицит по кормам |
...
*Dok 3 RPC Catalog v1.5 | TURAN AgOS | 22 June 2026*
*SQL source of truth: d01_kernel.sql, d02_tsp.sql, d03_feed.sql, d04_vet.sql, d05_ops_edu.sql, d07_ai_gateway.sql, d09_consulting.sql*
