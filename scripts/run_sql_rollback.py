...
def main() -> int:
    ...
    if apply_mode:
        ...
        # Принимаем и кириллическое, и латинское слово: подтверждение, посланное через
        # пайп на Windows, приходит в кодировке консоли, и кириллица там бьётся —
        # из-за этого осознанное «да» превращалось в отказ. Слово по-прежнему
        # обязательно: ослаблена кодировка, а не защита.
        ok_words = {"ПРИМЕНИТЬ", "APPLY"}
        try:
            answer = input("Введите ПРИМЕНИТЬ (или APPLY) для подтверждения: ").strip()
        except EOFError:
            print("ОТМЕНА: подтверждение невозможно в неинтерактивном режиме.")
            return 1
        if answer.upper() not in ok_words:
            print(f"ОТМЕНА: подтверждение не получено (пришло: {answer!r}).")
            return 1
