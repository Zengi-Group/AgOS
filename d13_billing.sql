...
--
-- This is deliberately a read-only projection. `membership_subscription` owns
-- subscription lifecycle/dates and fn_org_membership_active owns effective access;
-- the old memberships.level path remains visible only as an explicit legacy source.
-- organization_type_assignments remains classification metadata: it cannot make an
-- MPK verified. Verification evidence comes only from append-only verification_records
-- for the relevant MPK membership row. There is no association-number source in the
-- schema, so the projection returns JSON null rather than inventing one.
-- ------------------------------------------------------------
...
begin
    ...
    -- Prefer a live subscription. Without one, the newest terminal row is the truthful
    -- history after reload; terminal state names themselves are not chronological.
    ...
    if v_is_active
       ...
       and v_subscription.state in ('trialing', 'active', 'grace') then
        ...
        -- The predicate can only be true here through the frozen legacy level-stack.
        -- Do not leak a terminal subscription's dates into that legacy projection.
    ...
    -- This preserves only the historical schema convention: null means no assigner
    -- was recorded. A non-null assigner is provenance, not proof of operator approval.
    v_is_self_assigned := v_has_assignment and (v_assignment.assigned_by is null);

    -- A stale MPK membership/evidence row without current MPK classification must not
    -- leak into this projection or be mistaken for a live verification timeline.
    ...
    -- This is an evidence summary, not an admission or entitlement policy. In
    -- particular, an MPK type assignment without evidence is always incomplete.
...
values
    ...
    ('rpc_get_org_membership_verification', null, 'get_org_membership_verification', 'd13_billing.sql', 'ARS-361 canonical membership lifecycle + MPK verification read model'),
