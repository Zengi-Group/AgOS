...
export function useBatches(accountId?: string | null): UseBatchesResult {
  ...
  // Рекомендация цены (ARS-760, канон MS4 BT-09): свип гасит предложения по их
  // собственному сроку и переводит в 'decision' партию, которой рынок ОТКАЗАЛ —
  // среди предложений нынешнего выхода на рынок есть истёкшее и нет ни одного живого.
  // Возраст партии основанием не является: партия без предложений остаётся продаваться
  // и в цене не теряет. Ленивый вызов перед загрузкой. Ошибки/нет backend — тихо глушим.
