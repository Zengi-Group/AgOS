...
export interface Pool {
  ...
  minPoolHeads?: number
  // ARS-718 (Slice 11 M-012/M-016): раскрыты ли контакты поставщиков по этой заявке.
  // Поле УЖЕ приходит в ответе `rpc_get_my_pools` (`contactRevealed`) — до сих пор оно
  // просто терялось при маппинге. Решает раскрытие база (`D-M6-5/12`, ст. 171), экран
  // только показывает: выводить его на клиенте из наличия имени фермы значило бы завести
  // второй дом одного факта (`P4`). Аддитивно и необязательно — мобильный шелл поля не
  // читает и не меняется (`FR-014`).
