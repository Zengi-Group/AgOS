...
// Монограмма: «МК «Семей Ет»» → «СЕ». Кавычки в казахстанских наименованиях выделяют
// собственно имя (ТОО «X»), поэтому если они есть — инициалы берутся изнутри них.
function initials(source: string | null | undefined, fallback: string): string {
  if (!source) return fallback
  const quoted = source.match(/[«"]([^»"]+)[»"]/)?.[1]
  const words = (quoted ?? source).split(/\s+/).filter(Boolean)
