...
### 2026-07-11: Рынок — оффер вступления (гейт не-члена) в блоб-язык empty-state (R-20)
...
**Why**: правка CEO «улучши Empty State вкладки Рынок → офер на вступление». Экран был портом прототипа и не получил daylight-редизайн — пунктирная рамка прямо нарушала R-6/UI-EMPTYSTATE-01 («рамка → блоб») и §4 канона. Применено уже принятое правило R-6 к пропущенному экрану; зафиксировано новым правилом R-20 (гейты/оферы = блоб, не рамка), чтобы не повторилось.
...
**Files**: `src/pages/cabinet/shell/screens/MarketScreen.tsx`, `src/pages/cabinet/shell/market-proto.css`, `src/pages/cabinet/shell/cabinet.css`, `Docs/AGOS-DesignRules-FarmerCabinet.md` (R-20). Ветка `claude/market-offers-empty-state-903207` (worktree `phone-field-design-0cc909`).
