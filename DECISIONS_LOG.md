...
### 2026-08-27: QA-фикстур для МПК — `seed_mpk.mjs`

What: добавлен `scripts/seed_mpk.mjs` — идемпотентный сид тестового МПК-аккаунта (`+77010000002` / PIN `123456`) по образцу существующего `seed_farmer.mjs`: Admin API создаёт `auth.users` (phone+PIN как password), затем сессия под этим пользователем вызывает тот же `rpc_register_organization(p_org_type='mpk', p_bin=null, ...)`, что и настоящая регистрация (P-AI-1, zero duplication). БИН намеренно `null` — фиктивный БИН рискует случайно совпасть с реальным мясокомбинатом; `rpc_register_organization` проверяет уникальность БИН только если он передан (`d01_kernel.sql:3933`). Без стада/плана — `farms` не создаётся для `org_type='mpk'` (`d01_kernel.sql:3991`), это фермерская специфика.

Why: живая регистрация на реальном номере (оператор Beeline) сейчас блокируется — либо Mobizon отклоняет SMS для этого оператора, либо блокировка на стороне Beeline (диагностировано в сессии 2026-08-27 через чтение `supabase/functions/bird-otp/index.ts` + сравнение поведения между Incognito Chrome / Edge без автозаполнения / тестовой отправки на синтетический номер, где OTP прошёл штатно). CEO поручил дать возможность кликать кабинет фермера и кабинет МПК без прохождения SMS-флоу. МПК-фикстур уже был предусмотрен, но не сделан — `qa/README.md:120` заранее перечисляет ожидаемые seed-аккаунты («фермер-не-член, фермер-член(active), МПК, админ»); фермерский и админский сиды уже существовали (`seed_farmer.mjs`, `seed_admin.mjs`), МПК был единственным пробелом.

Consequences: с этим фикстуром можно логиниться в `/mpk` без SMS для ручной QA-проверки. Организация создаётся без БИН и без верификации — `MpkApp.tsx` покажет `verification.status = under_review` (дефолт `deriveMpkType`), это ожидаемо для синтетического аккаунта, не баг. Первопричина блокировки SMS для Beeline-номеров НЕ устранена этим изменением — она вне скоупа (внешний провайдер Mobizon/оператор, не код приложения) и остаётся открытым вопросом для отдельного расследования аккаунта Mobizon.

Files: `scripts/seed_mpk.mjs` (новый, ветка `dias/qa-seed-mpk`, ещё не закоммичен — ждёт ручной проверки автором перед коммитом).

### 2026-09-01: D-MPK-TYPE-01 — тип-шкала профиля МПК = шкала прототипа, не DS

What: при реализации `/mpk/profile/*` (эпик ARS-351) используется тип-шкала из прототипа `Docs/prototype/Кабинет МПК v4.dc.html`, а НЕ базовая шкала `Docs/design-system-v12/colors_and_type.css`. Расхождение по всем семи ступеням: `--fs-2xs` 10→11, `--fs-xs` 11→13, `--fs-sm` 12→13, `--fs-base` 13→15, `--fs-md` 14→16, `--fs-lg` 15→18, `--fs-xl` 20→24. Автор прототипа отметил это в самом файле как «осознанное отклонение от DS».

Why: пользователь профиля МПК (снабженец) работает за настольным монитором в рабочем режиме — читаемость приоритетнее плотности. Базовая шкала DS калибровалась под фермерскую мобильную зону, где ограничен экран. Решение принято Dias 2026-09-01 после сверки обеих шкал по `_ds_manifest.json`.

Consequences: (+) профиль совпадает с макетом пиксель-в-пиксель, визуальная приёмка `EngSpec §0` проходима. (−) в продукте появляются две тип-шкалы, поэтому переопределение ОБЯЗАНО быть scoped на контейнер профиля МПК (образец — `.agos-cabinet-stage` в `src/pages/cabinet/shell/shell-proto.css`). Задание токенов на `:root`, как в прототипе, недопустимо: увеличенная шкала и тёмная палитра протекут в `/cabinet` и нарушат `D-UI-FARMER-RULES-01` (daylight-канон). Проверка при приёмке — открыть `/cabinet` рядом с `/mpk/profile`: фермерский кабинет остаётся светлым, размеры шрифтов неизменны.

Files: `Docs/prototype/EXTRACT-tokens-and-data-map.md` (§2.3 — таблица сверки и решение, §2.8 — требование изоляции). Канон экранов — `Docs/AGOS-Dok6-Slice10-MPK-Profile.md` (ещё не написан).

### 2026-09-01: Версионирование прототипа МПК — макет в git, рантайм вне git

What: `Docs/prototype/` добавлена в репозиторий частично. В git: `Кабинет МПК v4.dc.html` (2280 строк, канон дизайна), `EXTRACT-tokens-and-data-map.md` (разбор), `_ds_manifest.json` (реестр токенов), `_adherence.oxlintrc.json` (правила линта DS), `README.md` (контекст бренда), `styles.css` (6 строк, точка входа). В `.gitignore`: `support.js` (1911), `agos-readable.js` (12 173), `_ds/**/_ds_bundle.js` (12 172), `_ds/**/colors_and_type.css` (256).

Why: два бандла — 24 345 из 29 674 строк (82% объёма папки) сгенерированного кода, который не читают и который восстанавливается повторным экспортом из Claude Design; версионирование дало бы шум в каждом дальнейшем диффе. `colors_and_type.css` из экспорта — копия `Docs/design-system-v12/colors_and_type.css` (значения идентичны, отличия только в аннотациях `@kind` и переводах строки CRLF/LF) → `P4`: один факт живёт в одном доме, две копии токенов неизбежно разъедутся.

Consequences: прототип не откроется корректно в браузере на свежем клоне (компоненты `Sidebar`/`Button`/`Icon` приходят из проигнорированного `_ds_bundle.js`) — инструкция восстановления в `EXTRACT-tokens-and-data-map.md` §2.7. Разработка при этом не блокируется: все значения выписаны в §1–2, токены — в `_ds_manifest.json` (в git). Ранее спека `AGOS-MPK-Profile-EngSpec-v0_1.md` §0/§11 называла прототип отсутствующим артефактом и блокировала визуальную приёмку — этот разрыв закрыт; канонический путь в спеке (`prototype/mpk-cabinet-v4.dc.html`) требует правки под фактический (`Docs/prototype/Кабинет МПК v4.dc.html`).

Files: `.gitignore`, `Docs/prototype/**`.

### 2026-09-01: D-MPK-NAV-01 — сайдбар профиля = все 6 пунктов; слаги URL = ключи прототипа

What: два решения по каркасу консоли `/mpk/profile/*` (эпик ARS-351, задача MP-3.1/ARS-623). (1) **Сайдбар** показывает все шесть пунктов прототипа (Главная · Мои заявки · Входящие офферы · Маркет-борд · Профиль МПК · Документы сделок); активен только «Профиль МПК», остальные пять ведут на состояние «Раздел в разработке», плюс ссылка «← Вернуться в закупки» → `/mpk`. (2) **Слаги URL** = внутренние ключи прототипа: `/mpk/profile/{overview,org,adm,team,rep,appeals}`.

Why: (1) кабинет будет достраиваться до полного вида, полный сайдбар сразу показывает карту продукта, а добавление разделов позже не требует переделки навигации (`P7`). Отвергнуты: только профиль + «назад» (скрывает будущую структуру); пункты, ведущие в мобильную оболочку (переход десктоп→телефонный интерфейс ломает ощущение консоли). (2) один набор идентификаторов вместо двух — нет таблицы перевода ключ↔URL, нечему разъезжаться (`P6`). Отвергнуты развёрнутые слаги (`enterprise`/`admission`/`reputation`): читаются лучше, но требуют слоя трансляции в `mpk/nav.ts`, то есть лишнего места для рассинхрона. Решения приняты Dias 2026-09-01.

Consequences: (1) на 272px-полосе пять из шести пунктов неактивны — визуально это принимается как осознанная плата за видимую карту продукта. ⚠️ Четыре из пяти разделов **существуют в мобильной оболочке**, поэтому копирайт «Раздел в разработке» корректен только в смысле «десктопная версия»; если формулировка может быть прочитана как «функции нет вообще» — уточнить до «Десктопная версия в разработке · доступно в мобильном кабинете» (открытый вопрос Slice10 §13 п.5, не блокирует старт). (2) deep-link менее нагляден (`/adm` вместо `/admission`), зато `mpk/nav.ts` не содержит словаря перевода.

Files: `Docs/AGOS-Dok6-Slice10-MPK-Profile.md` (§1.1 маршруты, §2 сайдбар, §13 статусы вопросов). Реализация — ARS-623 (MP-3.1).

### 2026-09-01: Иконки профиля МПК — 7 контуров дописать, согласование не требуется

What: проверено, что `src/pages/cabinet/shell/components/icons/PhIcon.tsx` — не пакет Phosphor, а собственный набор из 45 иконок, где каждая записана строкой SVG-контура, портированной из Phosphor вручную. Из 16 имён прототипа: 5 совпадают (`users`, `alert`, `check`, `clock`, `plus`), 2 существуют под другими именами (`messageCircle`→`chat`, `beef`→`cow`), 2 требуют уточнения начертания (`star`→`starOutline`, `shield`→`shieldCheck`), 7 отсутствуют и дописываются (`dashboard`, `building`, `fileText`, `mail`, `barChart`, `folder`, `download`).

Why: вопрос был сформулирован как «какую Phosphor-иконку выбрать вместо какой из DS» и считался блокером MP-3.1, требующим согласования с дизайном. Фактически библиотека одна и та же (Phosphor), различаются только имена и наличие контуров в локальном наборе — то есть задача механическая.

Consequences: блокировка MP-3.1 по иконкам снята. Остаются два вопроса начертания (`star` контурная или залитая, `shield` с галочкой или без) — решаются при постройке, не требуют отдельного согласования. Правило `D-UI-FARMER-RULES-01` (только Phosphor через `PhIcon`, никакого lucide в зоне кабинета) соблюдается по построению.

Files: `Docs/AGOS-Dok6-Slice10-MPK-Profile.md` §13.1.

### 2026-09-03: MP-3.1 (ARS-623) — SCR-P0, оболочка десктопной консоли профиля МПК

**What**: построена оболочка `/mpk/profile/:tab` — маршруты на шесть подразделов, 272px сайдбар, шапка профиля, ptabs, scoped-тема. Смонтирована **отдельным v6-роутом** `/mpk/profile/*` до общего `/mpk/*`, а не внутри Ionic-острова: `MpkApp.tsx` и его экраны/модалки/шторка не изменены ни строкой (`HS-2`/`FR-005`), общие `mpk/nav.ts` и `mpk/types.ts` расширены аддитивно (`P7`). SQL и RPC не менялись. Шесть разделов — честные заглушки (`FR-008`), их тела приходят с `ARS-624`…`631`.

**Why**: §1.2 спеки предписывала расширить в том числе outlet Ionic-острова, но десктопной консоли не нужен ни `IonApp`, ни телефонный каркас `.phone`, ни анимации стека; рендер внутри них означал бы переписывание `MpkApp` против `HS-2`. Отступление от буквы §1.2 вынесено на ревью архитектора отдельным пунктом. Изоляция темы (`FR-006`) сделана по образцу `.agos-cabinet-stage`: токены на контейнере, ни одного правила на `:root` — `/cabinet` daylight-палитру сохраняет.

**Consequences**: консоль достижима **только по прямой ссылке** — точка входа с `/mpk` намеренно не построена, это `MP-3.8`/`ARS-627`. Карта URL↔маршрут теперь одна на обе поверхности, но ветка `profile` в ней недостижима из `MpkApp` (следствие выбора монтирования; оценено `low`, оставлено). Правило доступа к закупкам продублировано в `admissionBadge`, а не импортировано из `MpkApp` — импорт затащил бы Ionic-остров в чанк консоли; натяжение с `P4` вынесено на ревью.

**Три дефекта нашли не автотесты**, и это главный урок прогона: (1) монограмма давала «ТQ» — первая буква из организационно-правовой формы (нашла проверка против живой БД; все 28 тестов шли на названии из прототипа с кавычками, где правило работало); (2) приглушение непостроенных пунктов давало в светлой теме контраст 2.49:1 при норме 4.5 — приглушение было моей добавкой, не заказанной ни одним требованием, поэтому убрано, а не подкручено; (3) сайдбар и вкладки оказались в размерах дизайн-системы — при переносе подставлена ступень `--fs-xs` вместо `--fs-base` (число 13px совпало, ступень разошлась), чем молча обнулялось решение `D-MPK-TYPE-01`. Второй и третий нашёл владелец на экране. **Общий вывод**: визуальная приёмка привязана к `theme=dark` (критерий 10), у светлого режима критерия нет вовсе — записано как корень, уезжает к `FR-019` на MP-3.8. **И правило на будущее**: размеры из DS-компонентов переносить только через ступень шкалы, никогда буквально в пикселях.

**Отложено решением владельца 03.09** к MP-3.8 (`ARS-627`) одной будущей поправкой замысла: `FR-018` — формулировка подсказки неактивных пунктов (выводит `FR-013` из обращения; контракт запрещает переписывать текст требования на месте), `FR-019` — тема консоли вместе с сохранением выбора, что заодно даёт id переключателю, который converge пометил как никем не заказанный. Причина переноса: распечатывать подписанный гейт ради текста, которого до выпуска никто не увидит, дороже пользы.

**Verification**: `test:routers` 29/29 (5 файлов, прежние `router-smoke`/`router-tabstack` целы), `test:unit` 8/8, `tsc -b` 0, `build` ok, `cross_check.sh` 0 critical. Строки матрицы закрыты тестами **по id**: `M-001`…`M-004`, `M-011`, `M-013`…`M-015`; `M-012` закрыта **частично** — построена застава, а не мост §1.4 (тело — `ARS-629`). Якорь 7 прошёл на три шага из четырёх: сценарии `06-tsp-mpk` не прогонялись решением владельца (`IMPL_DEBT` QA-SCENARIOS-MPK-DEFER-01), вместо них — проверка консоли против живой БД под сид-аккаунтом, которая и нашла дефект монограммы.

**Files**: `src/App.tsx`, `src/pages/cabinet/shell/mpk/{nav.ts,types.ts}`, `src/pages/cabinet/shell/mpk/profile/{MpkProfileApp,ProfileSidebar,ProfileTabs,SectionStub}.tsx`, `profile-console.css`, `src/pages/cabinet/shell/components/icons/PhIcon.tsx`, `src/tests/mpk-profile-{router,states}.browser.test.tsx`, `Docs/AGOS-Dok6-Slice10-MPK-Profile.md`, `IMPL_DEBT.md`. PR #176 (squash `92fc66b`).

### 2026-09-03: PWA — выкладка не доходит до пользователей без жёсткой перезагрузки (PWA-STALE-VERSION-01)

**What**: зафиксирован дефект уровня продукта, найденный сразу после выкладки MP-3.1: пользователь, у которого сайт уже открывался, **продолжает работать на старой версии после каждого релиза**. Три причины вместе: `navigateFallback: '/index.html'` (SW сам отвечает на любую навигацию из precache), отсутствие `skipWaiting`/`clientsClaim` (новый SW ждёт, пока закрыты все вкладки), и регистрация без обработки обновления (`HostContext.tsx:44-51` — нет подписки на `updatefound`/`waiting`, нет предложения обновиться).

**Why записано, а не починено**: выбор стратегии — продуктовое решение, не техническое. Тихое авто-обновление перезагружает страницу под пользователем и может прервать заполнение формы (для длинных форм MP-3.3 это реальная потеря ввода); предложение «доступна новая версия» безопаснее, но требует UI в обеих зонах. Трогать общую оболочку приложения в рамках MP-3.1 было бы выходом за скоуп.

**Consequences**: до реализации каждая выкладка требует предупреждать пользователей о жёсткой перезагрузке. Опаснее прямого неудобства то, что **симптом маскируется**: новый маршрут в старом бандле не существует, `/mpk/profile/*` перехватывается общим `/mpk/*`, и это выглядит как «выкладка не доехала» или «сломался роутинг». Именно так дефект и был первично истолкован 03.09, прежде чем проверка бандла на сервере показала, что код на месте.

**Files**: `IMPL_DEBT.md` (PWA-STALE-VERSION-01); поверхности — `vite.config.ts` (`VitePWA.workbox`), `src/platform/host/HostContext.tsx:44-51`.

### 2026-09-03: MP-3.3 (ARS-624) остановлена на якоре 6 — у раздела «Предприятие» нет read-пути

**What**: заход `/feature:build MP-3.3` прекращён **до первой правки кода** (от точки отсчёта `d0f87cc` в `src/` не изменено ничего). Причина: ARS-624 закрывает `FR-007` («одна read-model на превью и на фермерскую выдачу») и приёмку 3 («сетевой ответ до `confirmed` не содержит закрытых полей — тест на payload»), а читать раздел `org` нечем. Пять RPC ARS-359, названные в §4/§9 как покрытие, — **писатели**; читателя нет ни одного, а `mpk_profiles`/`mpk_sites`/`org_bank_accounts`/`org_field_reviews` закрыты от роли `authenticated` (`revoke all` при существующих SELECT-политиках именно для неё). Клиенту по `org` доступны только `legal_name`/`bin_iin`/`region_id`/`phone` из `rpc_get_my_context` плюс `fn_org_has_permission` для read-only-состояния (`FR-016`/`M-019`). Карточки организации для фермера не существует ни закрытой, ни раскрытой: `buyer`/`buyerPhone` в `src/pages/cabinet/shell/types.ts` — тип без бэкенда, токен `buyer` не встречается ни в одном SQL-файле.

**Why** (решение владельца, Dias, 2026-09-03): новой задачи под читателя не заводить — дом уже есть, **ARS-362** (`MP-2.1 · Read RPC: overview + organization profile`), в Scope дословно `rpc_get_org_profile`: canonical org/site/bank/contact + pending field reviews. Отвергнуто: (а) написать читателя внутри этого прогона — §4.2 юридически несущий (ст. 171 ПК РК), а «одна модель» требует спроектировать и фермерскую сторону, которой нет; проектирование на якоре 6 — ровно то, что процесс запрещает; (б) тонкий MP-3.3 на четырёх читаемых полях с заглушками на остальном — приёмка 3 не закрывается, загрузчик всё равно переписывается после ARS-362.

**Consequences**: ARS-624 возвращена Done → Backlog вручную (интеграция закрыла её по id в заголовке docs-PR #178, где кода MP-3.3 не было) и связана *blocked by* ARS-362. Точка отсчёта `d0f87cc` остаётся валидной для будущего прогона — она не израсходована. Следующий шаг — `/feature:feature` по ARS-362: у неё есть Scope и DoD, но нет слайс-спеки с замороженными требованиями и подписи G2. Порядок MP-3 сдвигается: ARS-362 встаёт перед MP-3.3.

**Заодно исправлены два ложных утверждения в документах** (они и назначили MP-3.3 строимой): Slice10 §9 и `EXTRACT-tokens-and-data-map.md` §4 «Итог» держали для `org` одно «✅ полное задеплоенное покрытие», перечисляя только писателей. Обе строки теперь называют отсутствие читателя и его дом. Мёртвые SELECT-политики без грантов (плюс безпотребительские `useMpkProfile`/`useOrganization`) записаны как `IMPL_DEBT` MPK-PROFILE-READ-PATH-01.

**Verification**: проверки продукта не гонялись — менять в коде нечего, диффа по `src/` нет. Утверждения о грантах и об отсутствии читателя сделаны **по файлам** (`d01_kernel.sql`, `supabase/migrations/20260730134113_ars_359_mpk_profile_data_model.sql`, `Docs/AGOS-MPK-Profile-EngSpec-v0_1.md` §8), а не по живой БД: `prod_diff.py` в этом заходе не запускался, последняя сверка — 2026-09-01, дрейф 0. Converge-обход живых слайсов (его требовал хук на старте сессии) не запускался: вердикт converge существует ради пакета G3, а G3 в этом заходе нет.

**Files**: `Docs/AGOS-Dok6-Slice10-MPK-Profile.md` (§9 + `Implementation Notes`), `Docs/prototype/EXTRACT-tokens-and-data-map.md` (§4 «Итог»), `IMPL_DEBT.md` (MPK-PROFILE-READ-PATH-01), `DECISIONS_LOG.md`; в мозге — `projects/agos/_project.md` (метка `Verify-набор:` → `Verify suite:`, чтобы её видел префлайт пакета).

### 2026-09-04: ARS-362 — гейт замысла читателя профиля: банк по праву, задача расщеплена, карточка фермера остаётся у ARS-624

**What**: заход `/feature` по ARS-362 (`MP-2.1`) дошёл до G2 и остановлен на подписи. Написана слайс-спека `Docs/AGOS-MPK-Profile-ReadRPC-ARS-362.md` (`FR-001`…`FR-013` · `M-001`…`M-009`, `status: draft`) на **один** RPC — `rpc_get_org_profile(p_organization_id uuid) returns jsonb`, `SECURITY DEFINER`/`STABLE`, один вызов на открытие вкладки «Предприятие»: организация · профиль · площадка · банк · правки на проверке · эффективные права. Кода нет, `baseline_commit` не ставился. Три решения владельца (Dias, 2026-09-04):

1. **Банк — только `mpk.bank.manage` или админ.** Конфликт вскрыт на якоре 3: строка §6.1 Slice10 обещала чтение банка «любому участнику», а задеплоенная политика `org_bank_accounts_read_authorized` (ARS-359, июль) пускает на SELECT только держателя права. RPC будет `SECURITY DEFINER` — политику он не спрашивает, значит решает тело. **Отвергнуто:** буква §6.1 (развернуло бы осознанный least-privilege мимоходом, в read-RPC, без ADR); маска «банк + последние 4 знака IBAN» всем (ближе к духу «видит, но менять не может», но вводит третье состояние отображения, которого нет в матрице `M-001…M-019`).
2. **ARS-362 расщеплена.** Агрегат «Обзора» (`rpc_get_mpk_profile_overview`) вынесен в **ARS-646** (`MP-2.2`) вместе со связью *blocks* ARS-628 и сводкой допуска для ARS-629; в Scope ARS-362 оставлен указатель, куда делась половина. **Отвергнуто:** оставить ARS-362 частично открытой — задача перестаёт быть join'ом «замысел↔работа», ровно так уже произошло ложное закрытие ARS-624; строить оба RPC — заход перестаёт быть заказанным и тянет за собой d02/d12/d13.
3. **Карточка МПК для фермера (`FR-007` Slice10, §4.2) в заход не входит** — решение ожидается от Аршидина 07.09. Читатель строится **плоским self-read'ом**, без общей проекции. **Отвергнуто:** писать `fn_org_card_projection` сейчас — у неё ровно один вызывающий, а пишется она ради второго, которого нет (`HS-4`), и её содержимое и есть ожидаемое решение о раскрытии (ст. 171 ПК РК); ждать 07.09 целиком — MP-3.3 стоит ещё неделю без нужды.

**Why**: у ARS-362 были Scope и DoD, но не было замысла с замороженными требованиями и подписи G2 — без них `/build` не стартует, а MP-3.3 стоит на ней (PR #179).

**Consequences**: `FR-016` Slice10 не тронут — граница заморозки строки 35–133, §6.1 стоит вне её и сам является раскрытием `FR-016`; строка §6.1 про **чтение** банка исправлена как ложное утверждение о реальности (написана в сентябре без сверки с политикой, задеплоенной в июле), `Spec Change Log` пуст, G2 Slice10 не переподписывается. В UI блок банка не скрывать — показывать с пояснением «реквизиты доступны бухгалтеру и администратору». Заведён долг **MPK-CARD-PROJECTION-01**: когда карточка фермера появится, проекция раскрытия строится один раз и тело `rpc_get_org_profile` переводится на неё (`P7` защищает сигнатуру и форму ответа, не тело) — обязательство продублировано строкой в ARS-624, где карточка и живёт. Спека вышла за бюджет 900–1600 токенов (~1900 слов) — предъявлено на гейте, владелец выбрал оставить как есть: вынос формы ответа в отдельный файл развёл бы контракт и требования по двум домам. **Правка владельца на гейте:** `FR-014`/`M-012` изначально привязывали замер p95 к staging-фикстурам, которых не существует (`IMPL_DEBT` QA-ENV-ISOLATION-01 — среда одна, `DATABASE_URL` не задан); требование оказалось бы измеримым только там, где его негде измерить, и на G3 кончилось бы либо waive, либо замером по боевой вопреки формулировке. Обе строки переписаны на сид-данные **среды приёмки** с обязательством пакета G3 назвать условия замера; опровергатель («появится изолированная среда») записан в `Assumptions`. Правка сделана **до** подписи — после неё она стоила бы переподписи гейта. ARS-624 возвращена In Progress → Backlog: работа по ней не идёт и не может, она заблокирована ARS-362, а висящий In Progress уже один раз довёл до ложного закрытия.

**Verification**: проверок продукта не гонялось — кода нет. Утверждения о грантах, политиках, отсутствии читателя и составе таблиц сделаны **по файлам** (`d01_kernel.sql`, `supabase/migrations/20260730134113_ars_359_mpk_profile_data_model.sql`, `20260731074740_ars_361_…sql`, `d12_messaging.sql`), а не по живой БД: `prod_diff.py` в этом заходе не запускался, последняя сверка 2026-09-01, дрейф 0. Converge-обход живых слайсов (его требовал хук на старте сессии) не запускался — вердикт converge существует ради пакета G3, а G3 здесь нет. Граф `graphify` пересобран в начале захода.

**Files**: `Docs/AGOS-MPK-Profile-ReadRPC-ARS-362.md` (новая), `Docs/AGOS-Dok6-Slice10-MPK-Profile.md` (§6.1 + `Clarifications` + §9), `IMPL_DEBT.md` (MPK-CARD-PROJECTION-01), `DECISIONS_LOG.md`; в трекере — ARS-646 (новая), ARS-362 (Scope/заголовок/связи), ARS-624 (обязательство по `FR-007`).
