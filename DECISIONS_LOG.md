...
**Verify**: `cross_check.sh` — 0 critical (4 пред-существующих significant: 3 TSP rpc_create_batch-оверлоада + R-29 дубль в design-canon, к этой работе не относятся); CHECK 3 (SECURITY DEFINER)/5 (P-AI-2: 3 subscription_id-RPC внесены в whitelist)/7 (registry, все 5 покрыты) чистые. Статическая сверка тел против живой схемы (колонки membership_plan.currency/billing_period, fn_current_user_id→uuid, publish_platform_event sig) пройдена. **Живой FSM-прогон НЕ выполнен**: у AgOS нет не-прод БД (биллинг-таблицы только в каноне d13, не в supabase/migrations → dev-branch их не содержит; проект Supabase = прод, ручной деплой). Прогон trialing→active→grace→past_due→(manual payment/extend восстанавливает)→revoked — за деплоем на прод (0 строк подписок, безопасно) при G3.
