...
**Deploy (2026-07-23, `mwtbozflyldcadypherr`)**: миграция `ars311_guard_d104_cascade_revoke_and_preview_wrapper` применена (`apply_migration` → success). Live-верификация ACL (реальный `has_function_privilege`, не SQL-строка — [[security-definer-review-checklist]]): `fn_shift_phase_cascade` и `fn_preview_cascade` → anon=false, authenticated=false; `proacl = {postgres,service_role}` на обеих (PUBLIC `=X/` снят, не только anon — Trap 2b закрыт). `rpc_preview_breeding_shift` → authenticated=true, anon=false. Живой вызов задеплоенной обёртки под seed-членом org (tx-local JWT) → 3 строки каскада (обёртка работает).

**Deploy-ordering follow-up (2026-07-24, решение CEO)**: живой фронт F8-превью зовёт `fn_preview_cascade` напрямую, а обёртка `rpc_preview_breeding_shift` приедет только с фронт-деплоем #142 → чтобы не сломать живой F8-превью, `fn_preview_cascade` **временно re-granted для authenticated** (миграция `ars311_temp_regrant_fn_preview_cascade_pending_frontend`; live ACL: `prev_auth=true, prev_anon=false`). Реальная дыра `fn_shift_phase_cascade` **ОСТАЁТСЯ закрытой** (`shift_auth/anon=false`); `fn_preview_cascade` self-guarded (L-7) → не живая утечка, только гигиена. Re-revoke после фронт-деплоя = **ARS-312** (Backlog, tier:mechanical). **Acceptance:** write-fn выполнен полностью сейчас; preview-fn — после ARS-312.
