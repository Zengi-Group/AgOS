...
### 2026-07-09: Редизайн фермерского приложения по прототипу agos-farmer (Phases 0–3)

**What**: Начат редизайн фермерского фронта под визуальный прототип `Zengi-Group/agos-farmer`. Готово и проверено: **Phase 0** (фундамент — ассеты `src/assets/turan/`, `@phosphor-icons/react@2.1.10`, `src/lib/auth-ui/tokens.ts`); **Phase 1** (вход-фаннел в светлой «paper»-теме — Welcome[native]/Login/Register[все 5 ролей] + `src/lib/auth-ui/primitives.tsx`, реcкин всех registration-шагов + shared PinInput/OtpInput/FloatingInput/BottomSheet/ChipSelect, логика `bird-otp`/`rpc_register_organization` сохранена); **Phase 2** (светлый экран заявки `src/pages/membership/Membership.tsx` + `rpc_submit_membership_application`+Storage, маршрут `/membership`); **Phase 3** (пере-темизация кабинета в светлую «daylight» DS + Home ПЕРЕПИСАН по структуре прототипа — новый `shell-proto.css` (порт CSS `home.jsx`/`shell.jsx`), `PhIcon`/`TuranStar`, HomeHead/HomeBanner[image-баннеры]/ServiceGrid[4]/TierHead/DecisionCard/ObserveCard/HomeScreen, таб-бар → 4 таба). Хендофф: `Docs/AGOS-Farmer-Redesign-Handoff.md`.

**Why**: CEO сделал новый прототип; воспроизводим 1:1 (цвета/компоненты/иконки/структура/контент). Урок: портировать СТРУКТУРУ прототипа, а не ре-темизировать старый разошедшийся кабинет.

**Consequences (решения)**: (1) **пока только СВЕТЛАЯ тема** кабинета, тёмная «night»+переключатель отложены (значения в `turan-tokens.css`); (2) **все 5 ролей** сохранены (не редуцируем до 2); (3) membership — отдельный `/membership` + шторки (реcкин в остатке Phase 3); (4) Welcome = native-вход, веб-лендинг `/` не тронут; (5) иконки — `@phosphor-icons/react` + inline `PhIcon`. Инвариант: RPC/данные/навигация/5 ролей/MPK не трогаем. `shell-proto.css` грузится после `cabinet.css` (additive-override). Осталось: Phases 4–8 (Рынок/Ферма/Сообщения/Сервисы-Магазин-Кабинет/MPK) + реcкин membership-шторок — тем же подходом.

**Files**: new — `src/lib/auth-ui/{tokens.ts,primitives.tsx}`, `src/pages/auth/Welcome.tsx`, `src/pages/membership/Membership.tsx`, `src/pages/cabinet/shell/shell-proto.css`, `src/pages/cabinet/shell/components/icons/{PhIcon,TuranStar}.tsx`, `src/assets/turan/*`, `Docs/AGOS-Farmer-Redesign-Handoff.md`; изменены — `src/App.tsx`, `src/index.css`, `src/pages/auth/Login.tsx`, `src/pages/registration/**`, `src/pages/cabinet/shell/{cabinet.css,ionic.css,CabinetApp.tsx}` + Home-компоненты + `screens/HomeScreen.tsx` + `components/ShellTabBarIon.tsx`. SQL не тронут.

### 2026-07-09: Редизайн — Phase 4 «Рынок» (4a фундамент + 4b Market/List)

**What**: Продолжение редизайна фермерского фронта по прототипу `agos-farmer` — модуль «Рынок» (Phase 4), под-фазы 4a+4b. **4a (фундамент, аддитивно)**: новый `src/pages/cabinet/shell/market-proto.css` = порт CSS рынка прототипа (база из `market-data.jsx` `#turan-market-css` 216 `.mk-*` + полиш `ds/turan-market-polish.css` 172 `.mk-*`), рескоуп всех селекторов в `.agos-cabinet-stage` скриптом-трансформером (`scratchpad/build_market_css.py`: `.sh-screen`/голые `.mk-*` → `.agos-cabinet-stage`; `@keyframes`/`@media` обрабатываются корректно; +keyframes `sh-fade`/`sh-sheet-up`); порядок база→полиш (равная специфичность, полиш побеждает). Импорт в `CabinetApp.tsx` ПОСЛЕ `shell-proto.css`. 15 market-иконок добавлены в `PhIcon` (minus/ban/trash/more/image/play/pencil/calendar/alert/userCheck/package/lock/info/checkCircle/starOutline). 5 tone-токенов daylight в `cabinet.css` `.agos-cabinet-stage` (`--cta-h`/`--green-m`/`--red-m`/`--amber-m`/`--blue-m`) — self-contained. **4b (Market + List)**: `MarketScreen` (таб «Рынок») переверстан → полный список «Мои партии» прототипа: `HomeHead` ask-бар + `.mk-tabs` (Все/В работе/Завершённые с счётчиками) + группы `.mk-grp` (Требуют решения/В работе/Завершённые) + ledger-карточки `.mk-card` + док-футер «Продать». Общий `components/BatchListCard.tsx` (`BatchCard`+`StatusChip`, DRY — Market и List). `ListScreen` (p1list) реcкин теми же `.mk-*`. `IonShellFrame` получил опциональные `footer`/`footBare` (аддитивно, P7); `.sh-foot` док-футер в `shell-proto.css` (порт `shell.jsx`). Проверено скриншотами в preview (mobile 375px) — совпадает с прототипом.

**Why**: Продолжение fixed-решения «воспроизвести прототип 1:1, портировать СТРУКТУРУ». Пользователь выбрал Phase 4 (Рынок) из хендоффа. Открытие: market CSS прототипа живёт в ДВУХ файлах (база в JSX-инъекции + полиш в DS) — портированы оба. Таб «Рынок» теперь = полный список (как в прототипе), а не компактный лендинг; p1list сохранён как вторичный вход (nav-инвариант). Лимит-5 перенесён в `renderMarket.onNew` (кнопка «Продать» на табе — основной вход в визард). Вся логика (gating SellGate/ApprovedPlate/expired, RPC, nav-коллбэки, лимит-5) сохранена — только презентация.

**Consequences**: CSS-фундамент рынка (`market-proto.css`) и иконки готовы → 4c (BatchScreen) / 4d (Wizard) / 4e (Review+Turan) — только переверстка JSX под `.mk-*`, без нового CSS. Осталось Phase 4: 4c/4d/4e + Phases 5–8. Подробности и гочи проверки в preview — в `Docs/AGOS-Farmer-Redesign-Handoff.md` (блок «🚧 Phase 4»).

**Files**: new — `src/pages/cabinet/shell/market-proto.css`, `src/pages/cabinet/shell/components/BatchListCard.tsx`; изменены — `src/pages/cabinet/shell/CabinetApp.tsx` (импорт market-proto.css + лимит-5 в renderMarket), `src/pages/cabinet/shell/cabinet.css` (+5 tone-токенов), `src/pages/cabinet/shell/shell-proto.css` (+`.sh-foot`), `src/pages/cabinet/shell/components/icons/PhIcon.tsx` (+15 иконок), `src/pages/cabinet/shell/components/IonShellFrame.tsx` (+footer/footBare), `src/pages/cabinet/shell/screens/MarketScreen.tsx`, `src/pages/cabinet/shell/screens/ListScreen.tsx`, `Docs/AGOS-Farmer-Redesign-Handoff.md`. SQL не тронут (`cross_check.sh` не нужен). Типы: `npx tsc -b --noEmit` — 0 ошибок. Ветка `claude/farmer-app-redesign-d5ff04` (Phase 0–3 влиты из d9bedf).

### 2026-07-09: Редизайн — Phase 4 «Рынок» 4c (BatchScreen)

**What**: Карточка партии `BatchScreen` переверстана под прототип `market.jsx BatchDetail`: заголовок `.mk-bz-head` (catName + heads/avg/район), вертикальный трекер жизненного цикла `.mk-trk` (порт `market-ui.jsx BatchPath` — focus+peek+expand, 5 стадий PATH_STAGES, `stageIndex(state)`), money-зона `.mk-money` (ЦЕНА СДЕЛКИ/ВАША ЦЕНА + lockchip), `.mk-buyer`, аккордеоны `.mk-acc` (Данные партии / История с `.mk-olist`), decision-композиция `.mk-dec-blk` (`.mk-rec` рекомендация + `.dec-row-actions.stack`), `SplitPanel` (Слайс 9) на `.mk-headsum`, kebab-меню вторичных действий (`.mk-kebab` → `ActionMenu` через общий `Sheet`/IonModal, `.mk-menu-item` danger/help). Проверено скриншотами: decision (b2) — трекер+рекомендация+3 действия колонкой; offering (b1) — трекер с дедлайном+money+kebab «Снять с продажи».

**Why**: Продолжение Phase 4 (пользователь прислал скриншот старого BatchScreen «не соответствует дизайну»). Реcкин — вся логика сохранена дословно: onPatch-сигналы `_withdraw`/`_dispatchReady`, prot-price валидация (custom-цена ≥ защитной — правило приложения, НЕ прототипный below-prot-с-подтверждением), deal-doc (печать), 3 шторки (Withdraw/Dispatch/BatchPrice), `host.haptics` на отгрузке. Prototype-специфики опущены осознанно: HeadsScreen/Поголовье-nav (в приложении нет heads-роута).

**Consequences**: Осталось Phase 4 — 4d (Wizard) + 4e (Review+Turan), только переверстка JSX. Побочно исправлено: `.dec-row-actions.stack` (вертикальная колонка decision-действий) не был портирован в Phase 3 из `turan-base.css` → добавлен в `shell-proto.css` (иначе кнопки уезжали за экран). `.mk-topbar` (back+kebab строка пушевых экранов) добавлен в `market-proto.css` — переиспользуется в 4e.

**Files**: изменён `src/pages/cabinet/shell/screens/BatchScreen.tsx` (полная переверстка, логика сохранена), `src/pages/cabinet/shell/shell-proto.css` (+`.dec-row-actions.stack`), `src/pages/cabinet/shell/market-proto.css` (+`.mk-topbar`), `Docs/AGOS-Farmer-Redesign-Handoff.md`. SQL не тронут. `npx tsc -b --noEmit` — 0 ошибок.

### 2026-07-09: Редизайн — аудит роутов/CTA фермерского shell + нативные анимации

**What**: Точечный аудит навигации нативного кабинета фермера + два фикса. **Аудит** (route-машина `nav.ts` × RouteV5-регистрации × все `go()`/`goTab()`/`goBackTo()`/CTA): все роуты и обратный маппинг `urlToRoute` консистентны; batch/list/review/turan/home/tab-bar/PubResult/LimitSheet CTA резолвятся верно (farm/messages/shop → Placeholder = ожидаемо для незастроенных фаз); prices-шторка и openAI смонтированы. **Найден 1 дефект (Significant)**: тайл «Сервисы» (`ServiceGrid` → `go({name:'services'})`) уводил на Главную — `routeToUrl` хардкодил `services`→`/cabinet` (алиас home), отдельного экрана/URL не было. **Fix 1**: `services` отвязан от home — `routeToUrl`→`/cabinet/services`, добавлен `urlToRoute` case + `DEPTH.services=1` (pushed) + RouteV5 `/cabinet/services`→`PlaceholderScreen "Сервисы"` (единообразно с «Магазин»→«Маркет»). **Fix 2 (анимации)**: подтверждено, что route→route переходы уже нативные (`setupIonicReact({mode:'ios'})` — iOS slide+parallax+swipe-back; таб-корни root/replace = мгновенно как нативные табы; шторки = IonModal drag). Единственный пробел — вход в Wizard/PubResult (swap состояния внутри `/cabinet/market`, не роут → Ionic не анимирует). Добавлена нативная modal-present анимация `.agos-flow-page` (`@keyframes agos-flow-present`: slide-up 16px + fade, 320ms, `cubic-bezier(.32,.72,0,1)`, + `prefers-reduced-motion` guard).

**Why**: Запрос CEO (/architect): проверить корректность роутов/CTA + сделать переходы нативными. «Сервисы» молча вёл на home — единственный неверный CTA. Wizard появлялся мгновенно (не как нативный экран). Оба — точечно, аддитивно (P7): роут services добавлен, не сломаны существующие; `tabOf` fallback `?? 'home'` покрывает подсветку таба.

**Consequences**: «Сервисы» ведёт на свой экран-плейсхолдер (реальный экран — Phase 7). Все CTA shell теперь резолвятся верно. Переходы нативны везде, включая вход в create-флоу. Wizard остаётся state-driven (не роут, ARS-157) → swipe-back на нём недоступен (арх. ограничение, вне «точечно»).

**Files**: `src/pages/cabinet/shell/nav.ts` (services routeToUrl/urlToRoute/DEPTH), `src/pages/cabinet/shell/CabinetApp.tsx` (+RouteV5 /cabinet/services), `src/pages/cabinet/shell/ionic.css` (+agos-flow-present). Проверено в preview: `/cabinet/services` рендерит плейсхолдер; wizard mount → `animation: agos-flow-present`. `npx tsc -b --noEmit` — 0 ошибок. SQL/каноны не тронуты.

### 2026-07-09: Редизайн — трёхуровневая система хедеров + аватар-иконка + фикс артефактов перехода

**What**: Четыре точечные доработки нативного кабинета. **(1) Аватар→иконка**: монограмма-аватар «АД» в тёмном кружке → тонкая минималистичная `PhIcon userThin` (Phosphor User thin) в общей кнопке `AccountBtn` (иконка + янтарная точка, → Кабинет). **(2) Фикс артефактов перехода**: у `.hh-row` (шапка Главной) убран `backdrop-filter: blur` + полупрозрачный фон → сплошной `var(--bg)`. Blur внутри страницы, которую Ionic двигает `transform`'ом при нативном переходе, ре-сэмплится и даёт артефакты (особенно заметно на home→Кабинет). **(3) Трёхуровневая система хедеров**: `HomeHead` (ask-бар — ТОЛЬКО Главная) + новый `TabHead` (универсальная шапка вкладок: крупный заголовок + `AccountBtn`) → Рынок/Ферма/Сообщения (ask-бар убран с не-домашних вкладок) + `SubHead` (порт прототипа `shell.jsx SubHead`: `‹ back-label` + опц. иконка/звезда/аватар + title/sub + right) → внутренние страницы. **(4) Внутренние хедеры**: BatchScreen (`.mk-topbar` → `SubHead` с back-label из `route.back` через `backLabelFor()` + kebab), ReviewScreen (глиф-back → `SubHead backLabel="Партия"`), TuranScreen (`.lst-head` → `SubHead` со звездой TURAN + «TURAN / Поддержка ассоциации»). Все хедеры sticky, сплошной фон (без blur).

**Why**: Запрос CEO (что осталось). Аватар-монограмма визуально тяжёлая — заменена на тонкую иконку (правило [[icons-phosphor-only]], вес thin — по запросу «минималистичную»). Ask-бар «Спросить…» уместен только на Главной; на прочих вкладках — заголовок раздела (нативный iOS large-title). Внутренние страницы имели разрозненные back-строки (глифы `←`/`‹`, `.mk-topbar`/`.bat-back`/`.lst-head`) — унифицированы в один `SubHead`.

**Consequences**: Единая система хедеров (Home/Tab/Sub), консистентная и нативная. Ферма/Сообщения (плейсхолдеры) получили `TabHead` через проп `PlaceholderScreen tab`. `backLabelFor(route.back)` даёт нативную подпись «‹ Рынок»/«‹ Мои партии». CabinetScreen (Phase 7) оставлен на старой `.back-strip` (вне scope). Тела Review/Turan — старый стиль до 4e (заменён только хедер). Артефакты перехода устранены (проверено: home→Партия слайд чистый, home→Кабинет — чистый результат).

**Files**: new — `components/{AccountBtn,TabHead,SubHead}.tsx`; изменены — `components/{HomeHead,icons/PhIcon}.tsx` (+userThin), `shell-proto.css` (.hh-row без blur, .acct-btn, .tab-head, .sub-head), `screens/{MarketScreen,BatchScreen,ReviewScreen,TuranScreen,PlaceholderScreen}.tsx`, `CabinetApp.tsx` (backLabelFor + tab-плейсхолдеры). `npx tsc -b --noEmit` — 0 ошибок. Проверено скриншотами (Главная/Рынок/Ферма/Партия/Отзыв/TURAN/Кабинет). SQL не тронут.

### 2026-07-09: Фикс артефактов перехода (добито) — непрозрачные ion-page/ion-content

**What**: Прошлый фикс (убран `backdrop-filter` у `.hh-row`) был необходим, но НЕ полон — по видео CEO артефакт остался: во время нативного слайда контент предыдущего экрана просвечивал сквозь/поверх нового. Настоящая причина — `ion-content.agos-ion-content { --background: transparent }` (ionic.css): статически прозрачная страница показывала фон стейджа (тот же цвет — незаметно), но при переходе входящая прозрачная страница проезжала поверх уходящей, и сквозь неё был виден предыдущий экран (bleed-through). Фикс: `ion-content --background: var(--bg)` (непрозрачный) + страховка `.agos-cabinet-stage .ion-page { background: var(--bg) }`. Теперь каждая страница — сплошной непрозрачный слой; уходящая полностью скрыта входящей. Проверено computed-стилями (оба слоя `#f6f3ed`) + чистые результаты переходов home↔Кабинет.
...
### 2026-07-09: Мастер новой партии (TSP-1) — инпуты и хедер под стиль registration_flow

**What**: Форма подачи партии (`WizStep1Animals` и др.) визуально расходилась с фаннелом входа. Правки только CSS в `market-proto.css` (7 блоков), компоненты/логика/JSX не тронуты (HS-1/HS-6): (1) `.mk-input`/`.mk-seltrig` — бежевая заливка `--bg-m` → белый `--bg-c` + `1px --bd` (как `FloatingInput` регистрации); (2) фокус — янтарный glow → тёмная рамка `--fg`, без свечения; (3) степпер-трек `.mk-stp` → белый+рамка, кнопки ± → `--bg-m`; (4) сегмент «упитанность» `.mk-fat` → белый+рамка, выбранный чип → янтарный tint + inset-ring; (5) прогресс `.mk-wiz-prog` — 3px-пилюли с glow → плоские 2px `--amber` (сегментный прогресс как в `TopBar`); (6) `.mk-wiz-bar` — нижний разделитель `.5px --bd-s`; (7) `.mk-h1` — `font-family:var(--font-sans)`, убирает случайный serif-лик от глобального `index.css h1{@apply font-serif}` (весь кабинет — Geist; serif был багом, не брендом).

**Why**: Пользователь (скриншот): «поля инпутов должны быть идентичны registration_flow» + «хедер». Согласованы 2 направления через вопрос: инпуты — визуальная идентичность через CSS (без свопа на FloatingInput/BottomSheet — это был бы rewrite WizStep); хедер — язык `TopBar` регистрации. Палитра cabinet-stage (`--bg-c/--bd/--fg/--amber`) — та же тёплая-бумажная семья, поэтому перенос «трактовки» регистрации без смены токенов. Sticky для шапки не делал: скроллер — shadow-DOM `ion-content`, `.phone-scroll` = `overflow:visible`, sticky ненадёжен.
...
### 2026-07-09: Международный PhonePicker в вход-фаннеле (/register, /login)
**What**: Поле «Телефон» на /register (Contact) и /login заменено на новый `src/components/PhonePicker.tsx` — селектор страны (bottom-sheet с поиском, флаги в круге, локаль ru.json, приоритетный блок СНГ+ → алфавит), форматирование на лету через `AsYouType` (libphonenumber-js), авто-blur при полном номере. Добавлены пакеты `react-phone-number-input` + `libphonenumber-js` (npm, legacy-peer-deps; peer-tree чист, libphonenumber-js deduped). Добавлен `maskPhoneE164()` в `src/lib/phone.ts`.
**Why**: Запрос Arshidin — довести дизайн поля до прототипа agos-farmer; спека приложена (13 приоритетных стран, E.164 наружу).
**Контракт**: `formData.phone` (регистрация) и `phone`-стейт (Login) теперь хранят **E.164** («+77001234567»), а не «10 цифр без +7». Для KZ (страна по умолчанию) значение байт-в-байт == прежний `+7`+10 цифр → бэкенд `bird-otp` / `rpc_register_organization` / `signInWithPassword` получают то же, что и раньше. Валидация — `isValidPhoneNumber`. `+7${...}` убран из Contact.tsx (send/resend), Registration.tsx (p_phone), CreatePin.tsx.
**Флаг (P2)**: для не-KZ стран в OTP/регистрацию уходит E.164 выбранной страны — доставка SMS зависит от провайдера bird-otp. KZ-путь без изменений; поддержку не-KZ на бэке не проверял.
**Verify**: tsc `-p tsconfig.app.json` = 0; превью :5188 — оба экрана рендерят пикер, bottom-sheet + поиск («герман»→Германия +49), KZ-формат «700 123 4567», CTA активируется на валидном, Login→PIN с маской «+7 701 123 ••••». Консоль 0 ошибок. `PhoneInput.tsx` (registration/components) остался мёртвым — не трогал (вне scope).
**Files**: new `src/components/PhonePicker.tsx`; edited `src/lib/phone.ts`, `src/pages/auth/Login.tsx`, `src/pages/registration/steps/Contact.tsx`, `src/pages/registration/steps/CreatePin.tsx`, `src/pages/registration/Registration.tsx`, `package.json`, `.claude/launch.json`. Ветка `claude/phone-field-design-0cc909`.
...
### 2026-07-09: Экран Success регистрации (7/7 «Готово») — редизайн под чистый success-скрин

**What**: Экран после регистрации (`Success.tsx`, шаг `success` фаннела) переверстан из «дэшборд»-варианта (мини-чром организации + KPI-грид + чек-лист «первые шаги» + баннер заявки) в чистый центрированный success-скрин по прототипу: янтарный круг Ø80 с белой галкой (`Check` из auth-ui/primitives), крупный bold-заголовок (Geist 30/700), приглушённое описание (≤320px), CTA в `StickyDock`. Для фермера — 2 кнопки («Подать заявку на членство» → `/membership`, «Перейти в кабинет» → `/cabinet`), для остальных ролей — одна («Перейти в кабинет →»). Контент вынесен в `CONTENT: Record<RoleType,{title,body,route}>` (farmer/mpk/services/feed_producer/expert). Навигация CTA и роут-таргеты сохранены 1:1.
Дополнительно: (1) `Registration.tsx` теперь рисует `TopBar` и на шаге `success` (было `!isSuccess &&` — шапка скрывалась), с `hideBack` (назад после сабмита некорректно — форма очищена); (2) счётчик/прогресс сделан role-aware через `visibleSteps(role)` — `expert_docs` считается только у экспертов, поэтому фермер видит **7/7** (как в прототипе), эксперт — 8/8; (3) в `primitives.tsx` условие счётчика `N/total` больше не требует `!hideBack` — иначе на success (hideBack) счётчик пропадал.

**Why**: Запрос руководителя со скриншотом-прототипом: «измени welcome screen, в который попадает фермер после регистрации — сделай как на скрине». Скрин = чистый success (галка + «Почти всё готово» + текст про членство + 2 кнопки + шапка «Готово 7/7»). Прежний дэшборд-экран не соответствовал; удаляемые блоки (KPI/чек-лист/баннер) — презентационные плейсхолдеры (нули/статичный текст), реальной функциональности за ними не было, поэтому HS-2 не нарушен. Явный редизайн-запрос от Arshidin = авторизация (HS-5). Правки только презентационные, поведение регистрации/сабмита не тронуто.

**Verify**: `tsc -p tsconfig.app.json --noEmit` = 0. Preview E2E (Vite :5205, worktree, `node_modules` симлинкнут из основного репо; временный shim: старт фаннела с `step='success'`+`role='farmer'`, откачен): `/register` рендерит шапку «Готово · 7/7» с полным янтарным прогрессом (7 сегментов), h1 «Почти всё готово», текст и обе кнопки; mobile 375px — пиксельно совпадает со скрином; клик по primary → `/membership`; консоль 0 ошибок.

**Files**: edited `src/pages/registration/steps/Success.tsx`, `src/pages/registration/Registration.tsx`, `src/lib/auth-ui/primitives.tsx`. SQL/canon не тронуты → `cross_check.sh` не нужен. NB: контракт экрана Success (ONB-SUCCESS-ORPHAN-01) в Dok 6 описывал старый дэшборд-вариант — при следующей ревизии слайса синхронизировать с чистым success-скрином. Ветка `claude/welcome-screen-redesign-d093a8`.
