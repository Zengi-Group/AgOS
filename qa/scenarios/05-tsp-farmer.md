...
- **Ожидание:** «Снизить и предложить снова» (значение = текущая − шаг **из `tsp_config`**, ARS-755 M-010) → цена обновлена, партия снова offering (повторная рассылка), новый дедлайн 24 ч. Живые `pending`-предложения по старой цене гаснут в `withdrawn` до записи новой (ARS-755 FR-007/M-007).

#### TSPF-LIFE-03 · UNHAPPY · Подсказка ниже пола не предлагается, ввод — разрешён
`layer:ui` `canon:D-M6-3;D-TSP-8 AMEND ARS-755` `impl:BatchScreen decision` `auto:candidate:unit` `status:active`
- **Предусловие:** «текущая − шаг» < защитной цены (шаг — из `tsp_config.price_step_down_amount`, ARS-755 M-010).
- **Ожидание:** блок подсказки «Рекомендуем» не показывается — **система** не предлагает цену ниже пола. При этом **фермер** может назначить любую цену > 0 вручную: ниже защитной показывается предупреждение, кнопка остаётся активной, записывается ровно введённое (ARS-755 FR-002/FR-003, M-004). Остаются также «оставить и ждать» (→ published, цена не меняется) и снять с продажи.
- **Изменено ARS-755:** до слайса кнопка «своя цена» была **заблокирована** ниже пола, а это жёсткий запрет, которого канон не вводил («задать вручную (с soft warning)», MS6 §229).

#### TSPF-LIFE-04 · EDGE · Ручная смена цены (BatchPriceSheet)
`layer:ui+rpc` `canon:MS4-BT-06;D-TSP-8 AMEND ARS-755` `impl:BatchPriceSheet+rpc_lower_price` `auto:candidate:sql` `status:active`
- **Ожидание:** любая цена > 0 сохраняется как введена — выше текущей, ниже или та же (ARS-755 M-012); ниже защитной показывается «Ниже защитного уровня», кнопка остаётся активной; цена доезжает **до БД** и (для offering) идёт ребродкаст.
- **Изменено ARS-755:** до слайса шторка звала `rpc_update_price` — no-op-функцию («Цена фермера упразднена (ст.171)», наследие ретайрнутой price-less позиции адаптера, развёрнутой `D-TSP-MATCH-01`). Фермер видел тост «Цена обновлена», в БД не менялось **ничего**; сверх того «Сохранить цену» была disabled ниже пола. Обе точки ввода кабинета теперь пишут цену через `rpc_lower_price` (один дом правила, P4).
...
#### TSPF-LIFE-19 · EDGE · Вход в точку решения по цене (poll-driven)
`layer:ui` `canon:MS4:BT-09` `impl:rpc_self_review_due_batches` `auto:none` `status:active`
- **Ожидание:** переход offering → decision управляется **истечением предложений**, а не возрастом партии (ARS-760): партия уходит в точку решения, когда среди предложений нынешнего выхода на рынок есть истёкшее и нет ни одного живого. Партия, которой предложений не делали, остаётся на рынке сколько угодно долго и в цене не теряет. Свип по-прежнему poll-driven — срабатывает при открытом кабинете фермера (расписания нет, ARS-694).
- **Не проверять по конфигу:** `price_decision_after_minutes` в этот переход больше не входит. До ARS-760 сценарий утверждал обратное; конфиг остался сроком **рассылки** предложений и по-прежнему отдаётся ключом `afterMinutes` в ответе свипа.
