...
- **Шаги:** offering → истечение окна (симулировать сдвигом expires_at) → awaiting_price_decision → `rpc_lower_price` (−шаг) → published → повторно, в т.ч. **ниже защитной** и **выше текущей**.
- **Ожидание:** записывается ровно названная цена в любую сторону — клэмпа нет ни к «текущая − шаг», ни к полу (ARS-755 FR-001/M-001..004); шаг из конфига остаётся только **подсказкой** экрана, и система сама цену ниже пола не предлагает. Смена цены гасит живые `pending`-офферы (FR-007); «оставить цену» их не трогает (M-017).
- **Изменено ARS-755:** формулировка «повторно до клэмпа на защитной цене» описывала зажим, которого канон не вводил, — после FR-001 она стала ложью.
