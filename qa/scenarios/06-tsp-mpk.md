...
- **Ожидание:** партия, переполняющая MAX строки, НЕ матчится (overshoot категорийного MAX запрещён; `ARS-754` — привязка только целиком, отказ `BATCH_DOES_NOT_FIT`). Уточнение по живому прогону (Слайс 8): жёсткий потолок подтверждён и на общем тотале — overshoot тотала на последней партии (D-TSP-9) на практике НЕ допускается; формулировка "overshoot тотала допустим" устарела.
